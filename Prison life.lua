...
local oldGui = LocalPlayer:WaitForChild("PlayerGui")
    :FindFirstChild("PrisonLifePremium")
    ...
    TabButton.MouseButton1Click:Connect(function()
        ...
        for _, b in pairs(TabButtons) do 
            TweenService:Create(b, TweenInfo.new(0.2), 
                {TextColor3 = Color3.fromRGB(120, 120, 130)}):Play()
        end
        ContentContainer.Visible = true
        TweenService:Create(TabButton, TweenInfo.new(0.2), 
            {TextColor3 = Color3.fromRGB(0, 162, 255)}):Play()
    end)
...
local function CreateToggle(name, configKey, parentTab, layoutOrder)
    ...
    local checkColor = _G.Config[configKey] 
        and Color3.fromRGB(0, 162, 255) or Color3.fromRGB(40, 40, 45)
    Checkbox.BackgroundColor3 = checkColor
    Checkbox.Text = ""
    ...
    local circlePos = _G.Config[configKey] 
    ...
    Checkbox.MouseButton1Click:Connect(function()
        ...
        local targetColor = _G.Config[configKey] 
            and Color3.fromRGB(0, 162, 255) or Color3.fromRGB(40, 40, 45)
        local targetPos = _G.Config[configKey] 
            and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)
...
local function CreateSlider(name, configKey, min, max, parentTab, order, cb)
    ...
    local function updateSlider(input)
        ...
        local absoluteSizeX = Track.AbsoluteSize.X
local posX = math.clamp(input.Position.X - absoluteX, 0, absoluteSizeX)local newPercent = posX / absoluteSizeXlocal value = math.floor(min + (newPercent * (max - min)))_G.Config[configKey] = valueLabel.Text = name .. ": " .. tostring(value)Fill.Size = UDim2.new(newPercent, 0, 1, 0)if cb then cb(value) endendlocal connectionTrack.InputBegan:Connect(function(input)local inputType = input.UserInputTypeif inputType == Enum.UserInputType.MouseButton1or inputType == Enum.UserInputType.Touch thenupdateSlider(input)connection = UserInputService.InputChanged:Connect(function(ch)local chType = ch.UserInputTypeif chType == Enum.UserInputType.MouseMovementor chType == Enum.UserInputType.Touch thenupdateSlider(ch)endend)endend)UserInputService.InputEnded:Connect(function(input)local inputType = input.UserInputTypeif inputType == Enum.UserInputType.MouseButton1or inputType == Enum.UserInputType.Touch thenif connection thenconnection:Disconnect()connection = nilendendend)endCreateToggle("Silent Aim", "SilentAim", MainTab, 1)CreateToggle("Wall Check", "WallCheck", MainTab, 2)CreateToggle("Team Check", "TeamCheck", MainTab, 3)CreateToggle("Random Parts", "RandomizeParts", MainTab, 4)CreateSlider("Aim FOV", "AimFOV", 10, 500, MainTab, 5, function(v)_G.Config.AimFOV = vend)CreateSlider("Hit Chance", "HitChance", 1, 100, MainTab, 6, function(v)_G.Config.HitChance = vend)CreateToggle("Enable ESP", "ESP", VisualsTab, 1)CreateSlider("WalkSpeed", "WalkSpeed", 16, 120, MiscTab, 1, function(v)if LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") thenLocalPlayer.Character:FindFirstChildOfClass("Humanoid").WalkSpeed = vendend)LocalPlayer.CharacterAdded:Connect(function(char)local hum = char:WaitForChild("Humanoid", 5)if hum then hum.WalkSpeed = _G.Config.WalkSpeed endend)local function IsVisible(targetPart)if not _G.Config.WallCheck then return true endlocal ignoreList = {LocalPlayer.Character, targetPart.Parent}local raycastParams = RaycastParams.new()raycastParams.FilterType = Enum.RaycastFilterType.ExcluderaycastParams.FilterDescendantsInstances = ignoreListlocal hit = workspace:Raycast(Camera.CFrame.Position,targetPart.Position - Camera.CFrame.Position, raycastParams)return hit == nilendlocal function GetClosestPlayer()local closestPlayer = nillocal shortestDistance = _G.Config.AimFOVfor _, player in ipairs(Players:GetPlayers()) doif player ~= LocalPlayer and player.Character thenlocal character = player.Characterlocal head = character:FindFirstChild("Head")if _G.Config.TeamCheck and player.Team == LocalPlayer.Team thencontinueendlocal humanoid = character:FindFirstChildOfClass("Humanoid")if humanoid and humanoid.Health <= 0 then continue endif head thenlocal screenPos, onScreen = Camera:WorldToViewportPoint(head.Position)if onScreen thenlocal mousePos = UserInputService:GetMouseLocation()local distance = (Vector2.new(screenPos.X, screenPos.Y)- mousePos).Magnitudeif distance < shortestDistance thenclosestPlayer = playershortestDistance = distanceendendendendendreturn closestPlayerendlocal function GetTargetPart(character)if not character then return nil endif _G.Config.RandomizeParts thenlocal availableParts = {}for _, partName in ipairs(BodyParts) dolocal part = character:FindFirstChild(partName)if part and part:IsA("BasePart") thentable.insert(availableParts, part)endendif #availableParts > 0 thenreturn availableParts[math.random(1, #availableParts)]endendreturn character:FindFirstChild("Head") or character:FindFirstChild("Torso")endlocal ShootEvent = workspace:FindFirstChild("ShootEvent", true)or game:GetService("ReplicatedStorage"):FindFirstChild("ShootEvent")if ShootEvent thenlocal oldNamecalloldNamecall = hookmetamethod(game, "__namecall", function(self, ...)local method = getnamecallmethod()local args = {...}if self == ShootEvent and method == "FireServer"and _G.Config.SilentAim thenif math.random(1, 100) <= _G.Config.HitChance thenlocal targetPlayer = GetClosestPlayer()if targetPlayer and targetPlayer.Character thenfor _, shotData in ipairs(args) doif type(shotData) == "table" thenlocal randomPart = GetTargetPart(targetPlayer.Character)if randomPart and IsVisible(randomPart) thenshotData.Part = randomPartshotData.Cframe = randomPart.CFrameendendendendendendreturn oldNamecall(self, unpack(args))end)end
